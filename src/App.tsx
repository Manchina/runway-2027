...
import { PATTERN_LIST, SYLLABUS_DSA_PROBLEMS, TOTAL_DSA_PROBLEMS } from "@/data/initialDsaCurriculum"
...
const formatIstDate = (value: string) => new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", weekday: "short", month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true }).format(new Date(value))
const istDateKey = (value: Date | string) => new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Kolkata", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(value))
...
function Dashboard() {
  ...
  const completion = Math.min(100, Math.round(((dsaProblems.length / TOTAL_DSA_PROBLEMS) * .65 + (mastered / 16) * .35) * 100))
  ...
  const todaySchedule = useMemo(() => SYLLABUS_DSA_PROBLEMS.filter((p) => istDateKey(p.scheduledDate) === istDateKey(new Date())), [])
    ...
    <Tabs value={view} onValueChange={(value) => setView(value as View)} className="min-h-screen flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <TabsContent value="today" className="space-y-8">
          ...
          <div className="grid gap-4 md:grid-cols-3"><MetricCard label="Problems logged" value={dsaProblems.length} detail={`of ${TOTAL_DSA_PROBLEMS} in the curriculum`} icon={Code2} /><MetricCard label="Designs mastered" value={mastered} detail="of 16 system studies" icon={BookOpen} /><MetricCard label="Current streak" value={`${streakCount}d`} detail={doneToday ? "Activity recorded today" : "Ready for today's rep"} icon={Flame} /></div>
          {todaySchedule.length ? <Card><CardHeader><CardTitle>Today’s IST plan</CardTitle><CardDescription>{todaySchedule.length} {todaySchedule.length === 1 ? "problem is" : "problems are"} scheduled for today at 9:00 AM IST.</CardDescription></CardHeader><CardContent className="flex flex-wrap gap-2">{todaySchedule.map((p) => <Button key={p.id} variant="outline" onClick={() => openLogger(p.id)}>{p.title}<ArrowRight data-icon="inline-end" /></Button>)}</CardContent></Card> : null}
        ...
        <TabsContent value="practice" className="space-y-8">
          <PageHeading eyebrow="Practice library" title="Build pattern fluency." description="The full NeetCode 150 is scheduled at 9:00 AM IST, finishing on December 31, 2026." action={<Button size="lg" onClick={() => openLogger()}><Plus data-icon="inline-start" />Log practice</Button>} />
