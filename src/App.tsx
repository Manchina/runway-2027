...
import { ArrowRight, BookOpen, CheckCircle2, Cloud, Code2, Database, ExternalLink, Flame, LayoutDashboard, MoreHorizontal, Plus, Search, Settings2, Sparkles, Trash2 } from "lucide-react"
...
function Dashboard() {
  const { dsaProblems, hldWeeks, streakCount, lastCompletedDate, isCloudConnected, markProblemCleared, failProblemReview, toggleHldChecklistItem, updateHldWeek, deleteDsaProblem } = useRunway()
    ...
    <Tabs value={view} onValueChange={(value) => setView(value as View)} className="min-h-screen flex-col bg-background">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        ...
        <TabsContent value="review" className="space-y-8">
          <PageHeading eyebrow="Recall checks" title="Retrieval is the work." description="Return without hints. A clean re-solve earns the clear." action={<Badge variant={due.length ? "default" : "secondary"} className="h-9 px-3 text-sm">{due.length} due</Badge>} />
          <div className="space-y-4">{due.length ? due.map((p) => <Card key={p.id}><CardHeader><CardTitle className="flex items-center gap-2"><Badge variant="secondary">Tier {p.struggleTier}</Badge>{p.title}</CardTitle><CardDescription>{p.pattern} · logged {formatDate(p.dateLogged)}</CardDescription></CardHeader>{p.notes ? <CardContent><p className="rounded-lg bg-muted p-4 text-sm leading-relaxed text-muted-foreground">{p.notes}</p></CardContent> : null}<CardFooter className="flex flex-wrap justify-end gap-2"><Button variant="ghost" nativeButton={false} render={<a href={p.leetcodeUrl} target="_blank" rel="noreferrer" />}>Open problem<ExternalLink data-icon="inline-end" /></Button><Button variant="outline" onClick={() => failProblemReview(p.id)}>Need another day</Button><Button onClick={() => markProblemCleared(p.id)}><CheckCircle2 data-icon="inline-start" />Mark cleared</Button></CardFooter></Card>) : <Card><CardContent className="pt-6"><Empty><EmptyHeader><EmptyMedia variant="icon"><Sparkles /></EmptyMedia><EmptyTitle>Nothing waiting</EmptyTitle><EmptyDescription>Your recall queue is clear. This is the rare inbox zero that actually means something.</EmptyDescription></EmptyHeader><EmptyContent><Button onClick={() => setView("practice")}>Choose a problem</Button></EmptyContent></Empty></CardContent></Card>}</div>
          {upcoming.length ? <Card><CardHeader><CardTitle>Coming up</CardTitle><CardDescription>Scheduled recall checks.</CardDescription></CardHeader><CardContent className="divide-y">{upcoming.map((p) => <div key={p.id} className="grid gap-1 py-3 sm:grid-cols-[100px_1fr_auto] sm:items-center"><span className="text-sm text-muted-foreground">{formatDate(p.scheduledReviewDate)}</span><span className="font-medium">{p.title}</span><Badge variant="outline">{p.pattern}</Badge></div>)}</CardContent></Card> : null}
        </TabsContent>
      </main>
    </Tabs>
