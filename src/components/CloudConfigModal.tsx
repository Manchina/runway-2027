...
export const CloudConfigModal: React.FC<CloudConfigModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className="bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        ...
        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto font-sans">
          ...
          <div className="space-y-3 font-mono text-xs">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 text-xs bg-slate-950 border border-slate-800 rounded-xl text-white font-mono placeholder-slate-600 focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-mono text-slate-300 mb-1.5 flex items-center justify-between">
                ...
                <span className="text-[10px] text-slate-500">Matches RUNWAY_API_KEY</span>
              </label>
              <input
                type="password"
                placeholder="Enter your private bot-shield key"
                value={apiKey}
                onChange={(e) => setApiKey(e.target.value)}
                className="w-full px-3 py-2 text-xs bg-slate-950 border border-slate-800 rounded-xl text-white font-mono placeholder-slate-600 focus:outline-none focus:border-indigo-500"
              />
              <p className="text-[11px] text-slate-400 mt-1 font-sans">
                Requests without this key are rejected with 401 in &lt;2ms, preventing any internet crawlers from querying DynamoDB.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-mono font-semibold flex items-center gap-1.5 transition-all cursor-pointer"
            >
            ...
            {apiUrl && (
              <button
                ...
                disabled={isSyncing}
                className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-emerald-300 text-xs font-mono flex items-center gap-1.5 border border-slate-700 transition-all cursor-pointer"
              >
              ...
              <div className="space-y-1">
                ...
                {testResult.details && (
                  <div className="text-[10px] text-slate-400 font-mono">
                    Database: {testResult.details.database} &bull; Table: {testResult.details.table}
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-[11px] text-slate-400 space-y-1.5">
            <div className="text-white font-semibold flex items-center gap-1.5 font-mono text-xs">
              ...
              <span>AWS Zero-Dollar Free Tier Guarantee</span>
            </div>
            <p>
            ...
            <p>
              &bull; <strong>CloudFront:</strong> 1 TB free data transfer every month.
            </p>
          </div>
        </div>
        ...
        <div className="px-6 py-3.5 border-t border-slate-800 bg-slate-900 flex items-center justify-between">
          <span className="text-[11px] font-mono text-slate-500">
            Package: <code className="text-indigo-400">backend/dist/lambda.zip</code>
          </span>
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-semibold shadow-md cursor-pointer font-mono"
            >
              Save Settings
            </button>
          </div>
